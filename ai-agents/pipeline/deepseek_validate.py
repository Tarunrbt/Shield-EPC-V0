"""
DeepSeek validation stage.

Calls the DeepSeek API (OpenAI-compatible endpoint) to perform the checks
...
Stage 1 envelope.
Reads DEEPSEEK_API_KEY from the environment (set by the workflow from
GitHub Secrets) — same pattern as GEMINI_API_KEY and CLAUDE_API_KEY.
"""
...
SYSTEM_PROMPT = (
    "You are Stage 2 (DeepSeek) in an HSE automation pipeline. "
...
def main():
    parser = argparse.ArgumentParser(description="DeepSeek validation stage")
    ...
    try:
        deepseek_checks = json.loads(raw)
    except json.JSONDecodeError:
        print(f"::error::DeepSeek did not return valid JSON: {raw}", file=sys.stderr)
        sys.exit(1)

    checks = {
        "human_in_the_loop_respected": deepseek_checks.get("human_in_the_loop_respected"),
        "architecture_bypass_detected": deepseek_checks.get("architecture_bypass_detected"),
        "envelope_schema_valid": deepseek_checks.get("envelope_schema_valid"),
    }
